...
import { asthmaData as asthmaAttributeData } from "@/data/asthemaTypeAttributeData";

import { asthmaStakeholderData } from "@/data/asthmaTypeStakeholderData";
...
const AsthmaType = () => {
    ...
    // available countries (only keys that have data)
    const availableCountries = Object.keys(countryDrugData).filter((c) => {
    ...
    // Attribute table (asthemaTypeAttributeData)
    const attributeDataForCountry: any[] = asthmaAttributeData[selectedCountry] || asthmaAttributeData["All"] || [];
    const attributeColumnsSet = new Set<string>();
    attributeDataForCountry.forEach((t) => t.drugs.forEach((d: any) => d.attributes.forEach((a: any) => attributeColumnsSet.add(a.attribute))));
    ...
    attributeDataForCountry.forEach((typeGroup) => {
        ...
        typeGroup.drugs.forEach((drug: any, idx: number) => {
            const row: any = { asthmaType: idx === 0 ? asthmaType : "", treatmentOption: drug.drugName };
            attributeColumns.forEach((attrName) => {
                ...
                if (attr) {
                    ...
                    row[attrName] = { Positive: "0% (0)", Negative: "0% (0)", Neutral: "0% (0)" };
                }
            });
    ...
    const stakeholderDataForCountry: any[] = asthmaStakeholderData[selectedCountry] || asthmaStakeholderData["All"] || [];
    const stakeholderSet = new Set<string>();
    stakeholderDataForCountry.forEach((t) => t.drugs.forEach((d: any) => d.attributes.forEach((a: any) => stakeholderSet.add(a.stakeholder))));
    const stakeholderColumns = Array.from(stakeholderSet);

    const stakeholderRows: any[] = [];
    stakeholderDataForCountry.forEach((typeGroup) => {
        const asthmaType = typeGroup.asthama_type || typeGroup.asthma_type || typeGroup.asthmaType || "";
        typeGroup.drugs.forEach((drug: any, idx: number) => {
            const row: any = { asthmaType: idx === 0 ? asthmaType : "", treatmentOption: drug.drugName };
                        ...
                        <div className="grid grid-cols-1 gap-6 mb-6">
                            ...
                            <div className="bg-card p-6 rounded-2xl border-2">
                                <div className="flex items-center justify-between mb-4">
                                    <h3 className="text-lg font-semibold text-foreground">Asthma Types — Attributes</h3>
                                </div>
                                <DataTable
                                    data={attributeRows}
                                    columns={["asthmaType", "treatmentOption", ...attributeColumns]}
                                    sentimentColumns
                                />
                            </div>
                            ...
                            <div className="bg-card p-6 rounded-2xl border-2">
                                <div className="flex items-center justify-between mb-4">
                                    <h3 className="text-lg font-semibold text-foreground">Asthma Types — Stakeholders</h3>
                                </div>
                                <DataTable
                                    data={stakeholderRows}
                                    columns={["asthmaType", "treatmentOption", ...stakeholderColumns]}
                                    sentimentColumns
                                />
                            </div>
                        </div>
